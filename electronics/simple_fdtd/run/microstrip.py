...
microstrip_length = 10

ribbon_dielectric_constant = 2.9
ribbon_thickness = 1.58
fluid_dielectric_constant = 65

pcb = fd.PCB(0.0002, xy_margin=25, z_margin=25)
fd.initialize_grid(pcb,int((microstrip_width)/pcb.cell_size),int((microstrip_length)/pcb.cell_size),
...
pcb.grid[pcb.xy_margin:-pcb.xy_margin, pcb.xy_margin:-pcb.xy_margin, (pcb.component_plane_z+1):(pcb.component_plane_z+ribbon_thickness)] \
...
while(pcb.grid.time_steps_passed < (20 * 2.0 * pi * f)):
    ...
    source_resistive_voltage = (50.0 * pcb.component_ports[0].get_current())
    pcb.component_ports[0].set_voltage(source_voltage + source_resistive_voltage)

    source_voltage = 0
    source_resistive_voltage = (50.0 * pcb.component_ports[1].get_current())
    pcb.component_ports[1].set_voltage(source_voltage + source_resistive_voltage)
