...
import sys
data = np.genfromtxt(sys.argv[1], delimiter=",", dtype=np.float, encoding='ascii', skip_header=0)

data = np.vsplit(data, np.array(np.where(data[:,1] == 99)[0]))
#
# plt.plot(data[0][:,2])
# plt.plot(data[0][:,3])
# plt.plot(, data[0][:,3][np.where(data[0][:,3] > 0)[0][0]])
# plt.show()

for j in range(0,2):

    for i in range(0,4): # roll each column
        data[j][:,i] = np.roll(data[j][:,i], -np.where(data[j][:,3] > 0)[0][0])

    data[j] = np.vsplit(data[j], np.where(data[j][:,3] > 0)[0])
