...
valid_lines = []
with open('sample_present_max_power_sweep.csv', "r") as f:
    for line in f:
        if(line.count(',') == 15):
...
#
plt.plot(freqs,data[0][:,far_sensor])
plt.plot(freqs,data[0][:,near_sensor])


plt.plot(freqs,data[4][:,far_sensor])
plt.plot(freqs,data[4][:,near_sensor])
