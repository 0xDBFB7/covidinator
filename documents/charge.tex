...
\footnotetext{
	\cite{Icosahedral2019}: \begin{quote}
		Different from insect, plant, and human, viruses had more positively charged than negatively charged segments. Viral proteins were the only class enriched in extremely high positively charged segments (charge greaterthan + 17) when compared to the overall proteome (1B-inset with p-values).
	\end{quote}
}
...
\footnotetext{
	It should be noted that these are the simplest primary structure charges. The charge in practice is determined by the secondary structure and the bonding to adjacent structures. More precise charges appear to be somewhat inconsistent. [] Find that the isoelectric point of the M protein charge is so low that it runs off the end of their 2D electrophoresis gel. However, the predicted value by [] is about 3.
	This is not of much importance for here, but may be important when setting up MD simulations.
}

\footnotetext{

Because CryoEM uses charged particles, it is possible to extract the actual charge distribution of a structure directly from an un-fit \cite{Identification2018}.
