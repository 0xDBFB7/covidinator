...
\begin{abstract}
	...
	One interesting finding is that the minimum-amplitude waveform which maximally drives an oscillator through a lossy dispersive medium like tissue appears to be a simple sawtooth.\\
	
	%Another route that might be worth investigating further is that it 
	It could be worth investigating the plausibility of rupturing the virus by optical centrifuge; that is, by inducing electrorotation by synchronously chirping the polarization of a MV/m Brillouin pulse along with the virus; using the virion as a kind of electrostatic induction motor. There is little reason to believe that this will be effective, however.
	
	Given that every route pursued consumes precious resources and attention, this. In retrospect, this was probably not a good use of time.
...
%
%Due to an unhealthy preoccupation with methodology and instrumentation - things which, ultimately, have nothing to do with viruses; and so no base truths have been arrived at. 
%
