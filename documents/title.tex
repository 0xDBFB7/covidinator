...
\begin{abstract}
	It has been suggested that the charge distribution and damping of the virus might offer some scope for inactivation by extreme electric fields, through mechanical damage to the viral membrane via a mechanism similar to, but distinct from, established lipid electroporation effects. \\
	
	Due to wide error bars and an unhealthy preoccupation with methodology and instrumentation, we have not been able to contribute even a tenuous theoretical or experimental basis for, or against, such an effect. It is hoped that this work will be of some small use in suggesting some lines of inquiry for future experimenters, and reporting some pitfalls encountered.\\
	
	
	A series of haphazard experiments on bacteriophage T4 have yielded no novel results. With a field of $10^6$ V/m in the nanosecond pulse regime and $10^2$ V/m in the microsecond pulsed X-band regime, we have not replicated the conditions or results of previous studies, especially considering the lack of a lipid envelope in the chosen surrogate. A generous reader might regard this as a negative result. Pulse shaping, a central thesis of this study, was not employed due to a lack of feedback.
	
	Optimization suggests that in certain cases, due to the Brillouin precursors, the minimum-amplitude waveform to maximally drive an oscillator like the virion through a lossy dispersive medium like tissue appears to be simply a sawtooth.\\
	
	Some precedent exists for adjacent routes such as optical centrifuge and electrorotation or 
...
\begin{tcolorbox}
\noindent \small{A more detailed (but completely unfinished) paper, a reference database for convenience, the (admittedly garbage) codebase, and a few other notes are available at 
\href{https://www.github.com/0xDBFB7/covidinator}{github.com/0xDBFB7/covidinator} \textit{[currently private]}. This document 
may be updated as information changes; minor revisions will be posted to the version at GitHub first.\\

This document is written in a terse and inaccessible format, at a time when clear and broadly understandable language is particularly important\cite{Re2021}. It is hoped that it will be rewritten at some point in the future. 
