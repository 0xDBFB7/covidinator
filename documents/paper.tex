...
This is especially true with RF biophysics. The literature is littered with otherwise impeccably perormed research which was later demonstrated to be an artifact. \\
	...
	Additionally, other sterilization techniques may produce superior results with less faffing about and should be evaluated in the same context. Data on far UV [Buonanno 2017] indicates safety.\\
...
%
[Fr\"{o}hlich 1968] \textrightarrow \ [Fr\"{o}hlich 1980] \textrightarrow \ [Liu 2009] \textrightarrow \ ([Hung 2014] $\parallel$ [Yang 2015] $\parallel$ [Sun 2017])\footnote{We have probably missed several steps in this chain out of haste.} apparently culminates in the flagship work we focus on in this paper,

{\it Efficient Structure Resonance Energy Transfer from Microwaves to Confined Acoustic Vibrations in Viruses} [Yang 2015].
%

estabilishes that - unique to certain viruses, and apparently unlike human cell structures, as we shall see - coincidentally have just the right size, shape, stiffness, and net charge distribution to form a weak (Q=2) spherical dipole resonance mode which couples well to the microwave spectrum at approximately the X-band (6-10 GHz).

More critically, [Yang 2015] (in parallel with [Hung 2014]) theoretically model and then experimentally demonstrate that the electric field strength required to produce an amplitude of vibration sufficient to tear the lipid envelope of the order of magnitude of 100 V/m.

They demonstrate this with both a plaque and PCR assay, with fantastic agreement to the theoretical model.



This is a non-ionizing, non-thermal technique.

Microwave resonant absorption of viruses through dipolar coupling with confined acoustic vibrations
...
\footnote{This {\it confined} acoustic resonance is apparently distinct from common-and-garden pipe-organ acoustic resonance. Rather than simple covalent or Lennard-Jones interaction between particles, if [Fr\"{o}hlich 1968] is to be believed at these nanoscopic scales a sort of coherent wavefunction is set up among the particles of the virus which produces coupling modes not otherwise expected. We confess to not yet understanding this phenomenon; fortunately, while helpful, the details of how this mode appears is not critical to implementing this technique.}
...
\footnote{All values have been converted to $\text{W/m}^2$ to avoid confusion. 100 $\text{W/m}^2 = 50 \text{mW/cm}^2$}
