...
The frequency impedance of an antenna can be determined by applying a gaussian pulse to a hard voltage source on the feedline, running the simulation until the transients have all died out while logging the voltage and current at each timestep, and then taking the fourier transform of the voltage and current. This is much faster and 

However, a similar mismatch to the SPICE exists for FFT methods, but in the other direction.  The courant limit often demands fine timesteps, but since each FFT bin is $ f_bin = n_bin * (1 / (N_simulation * dt)) $ , the majority of the FFT bins exist into the hundreds or thousands of GHz, leaving no resolution in the low-frequency domain of interest unless $N_simulation$ is extremely large - even if all the transients in the simulation have died down, you still have to keep the sim running to make the FFT happy! 
...
But, in a personally staggering turn of events which will presumably be familiar to statisticans and preposterous to everyone else, a far simpler method is to {\it discard} 95\% of the data! Re-sampling some fraction of the current  

An equally simple method that seemed to produce better results in our case is to {\it pad }
...
Though uneven dt FFTs exist, the time step can be constant here .

A step impulse (1 first timestep, 0 otherwise) can also be used, though the performance is almost unusable worse.
