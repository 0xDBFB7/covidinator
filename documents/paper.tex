...
\begin{itemize}
  \item Applying a gaussian pulse to a hard voltage source - in our case, applied to a via connecting the path (or probe)
...
A correction factor due to the staggered magnetic field of the Yee lattice should be introduced; [Fang 1994]. Their $Z_2$ equation (correcting for spatial inaccuracies, but not temporal) was sufficient.
...
The type of feed-port may affect the convergence rate by introducing unphysical transients.

Simply monitoring the source current during the simulation is deceptive. Periodically monitoring the fourier transform 
