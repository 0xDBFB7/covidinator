...
For the RF safety limits set by standards organizations are themselves based on the assumption that no resonances exist in biological tissues, and (as we shall see), this claim is grounded in solid {\it in vitro} (albeit limited {\it in vivo}) evidence. 
...
While it is possible to use conventional Si devices at these frequencies, especially with second-harmonic techniques [Winch 1982], obtaining the required output power is not trivial.
