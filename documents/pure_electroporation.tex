...
\author{\footnotesize{Daniel Correia\orcidlink{0000-0002-9353-0216}}\footnote{Ontario, Canada; contact information in the ORCiD.}}
...
The most obvious difference is scale. All other properties of the system being equal, the standard Schwan equation for transmembrane voltage implies that the steady-state DC voltage is proportional to the size of the particle. In the case of a 100 nm virus, this would 400x difference, which is reflected precisely in the steady state numerical simulations that follow.
...
\begin{tcolorbox}
	...
	This transposition was only detected late in the project, and so some documentation may still contain the incorrect coefficients. Beware.
...
underactuated robotics, simultaneous stabilization\cite{results1984}\cite{Simultaneous2005}
...
\begin{tcolorbox}
	First instinct might be to try to solve for the waveform that sets alpha beta gamma to zero. 
\end{tcolorbox}
