...
Compare to ~5 $k_b T$ per protein for capsid binding \cite{Energies2012} \cite{Weak2002} and $ (\text{17 to 78 kJ/mol})  / (310 \text{ K}\ k_b) = $ 6 to 30 $k_b T$ for lipid bilayer pore formation \cite{Atomistic2014a}.
%17 or 78 kJ/mol / (boltzmannConstant * 310 K) / avogadroConstant = 6.5 to 30.3 kT


%(2*pi*(20 GHz))^2 * 100 MDa to N/m \approx 3000 N/m
% 1 nanonewton / (3000 N/m) = 3e-13 m

% (2*pi*(5 GHz))^2 * 60 MDa to N/m = 100 N/m
% 0.1 angstroms

% 1000 N/m = 0.01 Angstroms
...
It is common for. However, non-zero resonant frequency requires the ratio of the the loss in energy to the surroundings to the mass, to be greater than $b/(2m) < \omega_{res} \sqrt{2}$ \cite{Driven}; the solution to the harmonic oscillator equation is piecewise around this value. 
...
We are adamant that, in all cases cited, we believe their studies were well designed and conducted: the authors went to considerable and sufficient lengths to demonstrate its validity. 
\end{toolchain}
