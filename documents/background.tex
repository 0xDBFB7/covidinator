...
Viral electroporation specifically has been observed.
...
The thermal artifact  the energy \footnote{(43 cal/cc * (3 microliters)) / (3 milligrams * 4.2 J/(g K))}. The electrodes were un-insulated, so electrochemical effects are possible.

Temperature rise 

at a slow field of approximately $10^7$ V/m, in the Vaccinia poxvirus (a 300 nm enveloped dsDNA virus). \cite{AC2017}. Interestingly, the same effect is not observed in T4 \cite{Manipulation2013}. 
...
A wave propagating into tissue with $e_r=70$ has a refractive index $n=\sqrt{\mu/\epsilon}$, and is transmitted as $\frac{2 n_1}{n_1 + n_2}$; therefore about 0.2x the electric field is transmitted. This is of no concern for waveguide or bolus applicators, since the reflected energy is captured and returned.
